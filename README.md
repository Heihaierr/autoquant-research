...
- **用更科学的方法做实验**：不是拍脑袋试参数，而是按规范流程去找策略背后
  真正起作用的原理，再一步步验证。目标是找到一个放到未来也大概率还能赚钱
  的策略，而不是一条只在历史数据里凑出来的好看曲线。
- **验证和数据检查都很严格**：数据先过质量检查这一关，策略要在不同时间
  窗口、不同成本假设下反复测试，保证你看到的效果背后，那些数字本身站得住
  脚，不是算错了或者蒙对了。
- **先按你的情况定目标，不套模板**：会先问清楚你能承受多大波动、习惯怎么
  交易、账户实际能买什么，据此定一个合理的研究目标，后面所有决策都照着这个
  目标走，不是不管什么人都给同一个答案。
- **方法本身是从大量真实研究里沉淀出来的**：把常踩的坑提前写进了流程里，
  agent 不用从零开始试错，用更少的对话、更少的 token 就能跑到一个靠谱的
  结论。
